...
const store=new Map();
const base=()=>({version:'v1.6.0',settings:{score:720,minutes:60,listen:'',read:''},dailyStats:{},history:{},diag:{list:[],i:0,correct:0,byPart:{},answers:[],currentAnswered:false,audioPlayed:{}},practice:{total:0,correct:0,partStats:{}},errors:[],vocabKnown:[],vocabSeen:{}});
const ctx={console,APP_VERSION:'v1.6.0',data:base(),fresh:base,today:()=> '2026-09-14',save:()=>{},updateAll:()=>{},confirm:()=>true,Blob:function(){},URL:{createObjectURL:()=>'',revokeObjectURL:()=>{}},setTimeout:()=>{},localStorage:{setItem:(k,v)=>store.set(k,String(v)),getItem:k=>store.has(k)?store.get(k):null,removeItem:k=>store.delete(k)},document:{querySelector:()=>null,getElementById:()=>null,createElement:()=>({click:()=>{},remove:()=>{}}),body:{appendChild:()=>{}}}};
...
const raw={settings:{score:875,minutes:90,listen:440,read:435},practice:{total:123,correct:111,partStats:{}},history:{'2026-09-13':80,'2026-09-14':50},errors:[{q:'<img src=x onerror=1>',why:'test'}],vocabKnown:['allocate','mandatory'],dailyStats:{},diag:{answers:[]},vocabSeen:{}};
