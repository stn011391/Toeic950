...
assert(APP_VERSION==='v1.4.0','version mismatch');assert(QUESTION_COUNT===all.length,'QUESTION_COUNT mismatch');assert(QUESTION_COUNT>=7000,`need 7000+ questions, got ${QUESTION_COUNT}`);assert(new Set(all.map(q=>q.id)).size===all.length,'question IDs must be unique');
