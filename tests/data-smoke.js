...
const files=['data-core.js','data-listening.js','data-part5.js','data-part6.js','data-part7.js','data-expansion-core.js','data-expansion-listening.js','data-expansion-part5.js','data-expansion-reading.js','data-diversity-v2.js','data-diversity-cleanup.js','data.js'];
...
assert(APP_VERSION==='v1.5.0','version mismatch');assert(QUESTION_COUNT===all.length,'QUESTION_COUNT mismatch');assert(QUESTION_COUNT>=7000,`need 7000+ questions, got ${QUESTION_COUNT}`);assert(new Set(all.map(q=>q.id)).size===all.length,'question IDs must be unique');
...
const lowP5=new Set(QUESTIONS[5].filter(q=>q.difficulty<=800).map(q=>q.family));assert(lowP5.size>=85,`Part 5 720-800 needs 85+ real families, got ${lowP5.size}`);assert(QUESTIONS[5].length>=1380,'Part 5 bank too small');assert(QUESTIONS[2].length>=780,'Part 2 bank too small');assert(QUESTIONS[3].length>=1200,'Part 3 bank too small');assert(QUESTIONS[4].length>=1200,'Part 4 bank too small');
