...
const counts=Object.fromEntries(parts.map(p=>[p,QUESTIONS[p].length]));console.log('Question counts:',counts,'total=',QUESTION_COUNT);
assert(APP_VERSION==='v1.5.0','version mismatch');assert(QUESTION_COUNT===all.length,'QUESTION_COUNT mismatch');assert(QUESTION_COUNT>=7700,`need 7700+ questions, got ${QUESTION_COUNT}`);assert(new Set(all.map(q=>q.id)).size===all.length,'question IDs must be unique');
